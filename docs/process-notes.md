...
## 2026-08-19 — 0.16.0: roster, chain outcome, widget rendering
- **Done:** Three gaps where the server knew something the caller never saw. `roster` on `create_session` (HAR-1582) — several chain templates declare roles and could not bootstrap through this client at all. Chain outcome rendered instead of an unconditional "Session created!", every non-started status saying NOT STARTED plus the remedy. Widget rendering in both chat tools, since the facilitator mirrors widgets into prose and dropping the spec leaves "drag to rank these" with nothing to drag. 11 new tests. Published 0.16.0.
- **Decisions:** Non-started chain statuses name the fix, not just the failure — the caller here is usually an agent that can apply it. Rendering compares tool output to text rather than attempting a control.
- **State:** Merged, published, git and npm agree at 0.16.0 with `v0.16.0` tagged. Verified end to end against production with a clean A/B on `delphi-panel-chain`: no roster gives `roster_incomplete` and no chain binding; with a roster, three step rows and an auto-project.
- **Housekeeping:** Repo went from 11 remote branches to 1. `release/harmonica-mcp-0.15.1` was an unmerged version-only branch — 0.15.1 shipped from it without merging back, so package.json said 0.15.0 while npm served 0.15.1. Deleted; the `v0.15.1` tag preserves the commit. The other nine were all squash-merged, which ancestry cannot detect — `git merge-base --is-ancestor` reports every squash-merged branch as unmerged, so PR state is the only reliable signal.
- **Next:** npm needed an OTP by hand; a granular publish-scoped token would make the next release non-interactive.
